...
/**
 * Tests that change the shared level's time/weather each run in their own batch —
 * batches execute sequentially, so they cannot race each other's daylight settings.
 * State assertions run >= 2 ticks after setDayTime because skyDarken (isDay) and
 * sky light both update during the level tick, not synchronously.
 */
...
public final class SolarPanelGameTests {
    ...
    public static void solarPanelGeneratesDuringDay(GameTestHelper helper) {
        AdvancedSolarPanelBlockEntity panel = placePanel(helper, ASPBlocks.ADVANCED_SOLAR_PANEL.get(), AdvancedSolarPanelBlockEntity.class);
        setDaylight(helper.getLevel(), true);
        double[] start = new double[1];

        helper.runAtTickTime(2, () -> {
            panel.updateGenerationState();
            helper.assertValueEqual(panel.generationState, AbstractSolarPanelBlockEntity.GenerationState.DAY, "generation state");
            start[0] = energy(panel).getEnergy();
        });
        helper.runAtTickTime(12, () -> {
            AdvSolarGameTestAssertions.assertNear(helper, energy(panel).getEnergy() - start[0], 80.0, 8.0, "ten ticks of daylight generation");
            helper.succeed();
        });
    }
    ...
    public static void solarPanelDoesNothingWhenCovered(GameTestHelper helper) {
        ...
        helper.runAtTickTime(2, () -> {
            panel.updateGenerationState();
            helper.assertValueEqual(panel.generationState, AbstractSolarPanelBlockEntity.GenerationState.NONE, "covered generation state");
            start[0] = energy(panel).getEnergy();
        });
        helper.runAtTickTime(12, () -> {
            AdvSolarGameTestAssertions.assertNear(helper, energy(panel).getEnergy() - start[0], 0.0, 0.0, "covered panel generation");
            helper.succeed();
        });
    }
    ...
    public static void solarPanelNightGeneration(GameTestHelper helper) {
        HybridSolarPanelBlockEntity panel = placePanel(helper, ASPBlocks.HYBRID_SOLAR_PANEL.get(), HybridSolarPanelBlockEntity.class);
        ...
        helper.runAtTickTime(2, () -> {
            panel.updateGenerationState();
            helper.assertValueEqual(panel.generationState, AbstractSolarPanelBlockEntity.GenerationState.NIGHT, "night generation state");
            nightStart[0] = energy(panel).getEnergy();
        });
        helper.runAtTickTime(12, () -> {
            nightGenerated[0] = energy(panel).getEnergy() - nightStart[0];
            AdvSolarGameTestAssertions.assertNear(helper, nightGenerated[0], 80.0, 8.0, "ten ticks of night generation");
            setDaylight(helper.getLevel(), true);
        });
        helper.runAtTickTime(14, () -> {
            panel.updateGenerationState();
            helper.assertValueEqual(panel.generationState, AbstractSolarPanelBlockEntity.GenerationState.DAY, "day generation state");
            dayStart[0] = energy(panel).getEnergy();
        });
        helper.runAtTickTime(24, () -> {
            double dayGenerated = energy(panel).getEnergy() - dayStart[0];
            AdvSolarGameTestAssertions.assertNear(helper, dayGenerated, 640.0, 64.0, "ten ticks of day generation");
            helper.assertTrue(dayGenerated >= nightGenerated[0] * 7.0, "hybrid daylight rate should be about eight times its night rate");
            helper.succeed();
        });
    }
    ...
    public static void solarPanelEmitsToSink(GameTestHelper helper) {
        ...
        TileEntityElectricBatBox batbox = getBlockEntity(helper, PANEL_POS.east(), TileEntityElectricBatBox.class);
        ...
        helper.succeedWhen(() -> {
            ...
            helper.assertTrue(energy(panel).getEnergy() < initial, "panel storage should drain into the batbox");
        });
    }
    ...
    public static void solarPanelChargesItemInSlot(GameTestHelper helper) {
        ...
        helper.runAtTickTime(5, () -> {
            helper.assertTrue(ElectricItem.manager.getCharge(panel.charge.get(0)) > 0.0, "RE battery should gain charge");
            helper.assertTrue(energy(panel).getEnergy() < initialStorage, "charging should consume panel storage");
            helper.succeed();
        });
    }
    ...
    public static void solarPanelStoragePersistsNbt(GameTestHelper helper) {
        ...
        AdvancedSolarPanelBlockEntity restored = new AdvancedSolarPanelBlockEntity(BlockPos.ZERO, ASPBlocks.ADVANCED_SOLAR_PANEL.get().defaultBlockState());
        restored.loadWithComponents(nbt, helper.getLevel().registryAccess());
        AdvSolarGameTestAssertions.assertNear(helper, energy(restored).getEnergy(), 12345.0, 0.0, "restored panel storage");
        helper.succeed();
    }
    ...
    private static <T extends AbstractSolarPanelBlockEntity> T placePanel(GameTestHelper helper, net.minecraft.world.level.block.Block block, Class<T> type) {
