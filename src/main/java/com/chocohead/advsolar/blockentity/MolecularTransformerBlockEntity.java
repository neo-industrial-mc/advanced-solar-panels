...
public final class MolecularTransformerBlockEntity extends TileEntityBase
  ...
  public MolecularTransformerBlockEntity(BlockPos pos, BlockState state) {
    super(ASPBlockEntities.MOLECULAR_TRANSFORMER.get(), pos, state);
    comparator.setUpdate(
        () -> totalEnergy <= 0 ? 0 : Math.min(15, (int) (energyUsed * 15 / totalEnergy)));
  }
  ...
  private void tryStart() {
    ...
    for (var vanillaHolder :
        level
            .getServer()
            .getRecipeManager()
            .getAllRecipesFor(ASPRecipes.MOLECULAR_TRANSFORMER.get())) {
      ...
      if (recipe.getInput().matches(stack) && stack.getCount() >= count && output.canAdd(result)) {
        ...
        totalEnergy = recipe.getMetaData().getInt("energy");
        energyUsed = 0;
        recipeInput = stack.copyWithCount(count);
        recipeOutput = result.copy();
        input.put(stack.copyWithCount(stack.getCount() - count));
        energy.setCapacity(totalEnergy);
        setActiveState(true, false);
        setChanged();
        return;
      }
  ...
  protected void loadAdditional(CompoundTag tag, HolderLookup.Provider registries) {
    ...
    recipeInput = ItemStack.parseOptional(registries, tag.getCompound("recipe"));
  }
  ...
  public void saveAdditional(CompoundTag tag, HolderLookup.Provider registries) {
    ...
    if (!recipeInput.isEmpty()) tag.put("recipe", recipeInput.save(registries));
  }

  @Override
  protected void onLoaded() {
    super.onLoaded();
    if (!recipeInput.isEmpty() && level.getServer() != null) {
      double savedEnergy = energyUsed;
      ItemStack saved = recipeInput.copy();
      input.put(saved.copy());
      tryStart();
      input.put(ItemStack.EMPTY);
      energyUsed = Math.min(savedEnergy, totalEnergy);
      energy.setCapacity(Math.max(0, totalEnergy - energyUsed));
    }
  }
  ...
  public double getGuiValue(String name) {
    return "progress".equals(name) && totalEnergy > 0 ? energyUsed / totalEnergy : 0;
  }

  public String getInput() {
    return recipeInput.isEmpty() ? "-" : recipeInput.getHoverName().getString();
  }

  public String getOutput() {
    return recipeOutput.isEmpty() ? "-" : recipeOutput.getHoverName().getString();
  }

  public String getEnergyNeeded() {
    return Integer.toString(totalEnergy);
  }

  public String getEU() {
    return Integer.toString((int) lastEnergyGiven);
  }

  public String getPercent() {
    return totalEnergy <= 0 ? "0%" : String.format("%.1f%%", energyUsed * 100 / totalEnergy);
  }
